...
Page({
  ...
  changeSome(e){
    ...
    this.data._detail[type] = e.detail
    this.setData({
      _detail: this.data._detail
    })
  },

  //input===== set指定字段数据到_detail中
  inputSome(e) {
    let type = e.currentTarget.dataset.type
    this.data._detail[type] = e.detail.value
    this.setData({
      _detail: this.data._detail
    })
  },
  ...
  get_detail: function(_detailId) {
    return wx.$http.res['_interFace'].getById(_detailId).then(res => {
      if (!res.errCode) {
        if (res.data) {
          //  0 =>'0'
  ...
  postInfo: function(e) {
    ...
    if (this.data.type == 'create') {
      wx.$http.post('/api/_interface', params).then(res => {
        if (!res.errCode) {
          Toast.clear()
          Toast.success('创建成功')
          wx.navigateBack({
      ...
      wx.$http.put('/api/_interface', params).then(res => {
        if (!res.errCode) {
          Toast.clear()
          Toast.success('修改成功')
          wx.navigateBack({
  ...
  onLoad: function(options) {
    ...
    if (options.type == 'change') {
      ...
      this.get_detail(options._detailId).then(() => {
      ...
      success: res => {
        wx.uploadFile({
          ...
          success: res => {
            ...
            this.data._detail.imageUrl = data.data;
            this.setData({
              _detail: this.data._detail
            })
          }
        ...
        res.tempFilePaths.forEach(item => {
          wx.uploadFile({
            ...
            success: res => {
              ...
              this.data._detail.imageUrls.push(data.data);
              this.setData({
                _detail: this.data._detail
              })
            }
...
;
  ...
  // 删除多张图片
  deleteImages(e) {
    this.data._detail.imageUrls.splice(e.target.dataset.index, 1)
    this.setData({
      _detail: this.data._detail
    })
  },
